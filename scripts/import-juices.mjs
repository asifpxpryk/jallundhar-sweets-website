...
const CATEGORY_ID = "135";
const AISLE_KEY = "beverage:juices";
const FILE_PREFIX = "juices";
...
function discountedPrice(value) {
  ...
  return Math.max(1, Math.round(raw * 0.95));
}
...
for (const product of products) {
  ...
  const price = discountedPrice(product.price_range?.minimum_price?.final_price?.value);
  if (!src || price <= 0) {
